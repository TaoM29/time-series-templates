...
This template prepares hourly air-quality sensor measurements for exploration and principal component analysis (PCA). It compares complete-case PCA with a version fitted after linear interpolation, then reconstructs the measurements from a reduced set of components. Generated outputs and plots are intentionally omitted; run the accompanying R script to reproduce the analysis.
...
The UCI dataset uses `-200` for missing observations and stores dates and times separately. Preparation combines them into a Rome-local timestamp, removes empty columns and incomplete calendar days, converts sensor fields to numeric values, and plots each measurement on its own scale.
